...
<div class="header">
	<div class="title">
		<?php
			 ...
			 echo $detailImage.'&nbsp'. CHtml::link($model->name.' ('.$model->course_code.')',array(strtr('/course/view?code={code}',array('{code}'=>$model->course_code))));
		?>
	</div>
</div>
		...
		<div class="section-content">
			<?php $this->widget('zii.widgets.CDetailView', array(
				...
				'attributes'=>array(
					'course_code',
					'name',
					array('label'=>'Category','type'=>'raw','value'=>html_entity_decode(CHtml::link($model->category->name,array(strtr('/course/categories?id={id}',array('{id}'=>$model->category->uid)))))),
					'description',		
				),
			)); ?>
		</div>
